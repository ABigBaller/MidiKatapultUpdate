...
class Control {
  ...
  int maxvalue = 127;
  void up(){ledOn(x, y, activecolor);}
  void down(){}
  void on(){display(x, y, activecolor); }
  void off(){display(x, y, idlecolor);}
  void send(){}
  ...
  void chainTo(Control control, boolean send) {
    ...
    //consoledebug("Chaining "+this+" to "+control);
...
class GridSegment extends Control {
  ...
  void on() {
    println(this+"x="+this.x+" y="+this.y+" on()");
  ...
  void off() {
    println(this+"x="+this.x+" y="+this.y+" off()");
...
class XFader extends Fader {
  ...
  void faderAction(FaderSegment sender) {
    ...
    float amount = ((relx + quantum) / (float)xsize) * maxvalue;
    setValue((int)amount);
  }
...
class YFader extends Fader {
  ...
  void faderAction(FaderSegment sender) {
    ...
    float amount = ((rely + quantum) / (float)ysize) * maxvalue;
